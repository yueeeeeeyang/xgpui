...
//! `Tree` 标准树组件、`DataTable` 标准数据表格组件和 `DateTimePicker` 日期时间选择器。
...
/// 安装 xgpui 的默认应用级能力。
///
/// 当前安装内容包括默认主题状态、Lucide 图标字体、`TextInput`、`Textarea`、`Select`、`Tree`
/// `DataTable` 和 `DateTimePicker` 默认键盘绑定。
...
pub fn install(cx: &mut gpui::App) {
    ...
    components::data_table::register_data_table_key_bindings(cx);
    components::date_time_picker::register_date_time_picker_key_bindings(cx);
